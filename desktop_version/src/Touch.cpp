...
namespace touch
{
    ...
    static void setup_map_buttons(void)
    {
        ...
        buttons[TOUCH_BUTTON_MAP_QUIT_SAVE].y = 96 + 8;
        ...
        buttons[TOUCH_BUTTON_MAP_QUIT_SAVEEXIT].y = 96 + 32 + 8;
        ...
        buttons[TOUCH_BUTTON_MAP_QUIT_EXIT].y = 96 + 64 + 8;
    ...
    void refresh_buttons(void)
    {
        ...
        buttons[TOUCH_BUTTON_MAP_BACK].x = width - ((double)(240 - graphics.menuoffset) / 240.0) * (60 * scale);
        ...
        case TITLEMODE:
            if (game.menustart && dynamic_buttons.size() == 0) // If we're in the menu, and there's no buttons, show DPAD controls so we're not softlocked
            {
                buttons[TOUCH_BUTTON_LEFT].active = true;
                buttons[TOUCH_BUTTON_RIGHT].active = true;
            ...
            else if (!game.menustart)
            {
                // If we're not on the menu yet, we don't want to use buttons
                // This is a bit hacky, because menus get created before we're actually on the menu,
                // which means the buttons get created as well.
                // Maybe in the future, we can improve the menu system, but for now this is okay.
                use_buttons = false;
            }
            break;
        case TELEPORTERMODE:
    ...
    void render(void)
    {
        ...
        render_buttons(scale, true, 65, 185, 207);
    }

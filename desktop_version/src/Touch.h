...
struct TouchButton
{
    ...
    bool down; // Whether the button is currently being pressed down
    bool pressed; // Whether the button was initially pressed, meaning it was pressed down and not dragged onto
    ...
    bool ui; // Whether the button is in the UI layer or the game layer
    std::string text; // The text for the button, if it doesn't have an image
    SDL_Texture* image; // The image for the button, if any
    SDL_FingerID fingerId; // The finger currently pressing the button, if any
    TouchButtonType type; // The type of the button
    int id; // The ID of the button, mainly used for menu buttons
